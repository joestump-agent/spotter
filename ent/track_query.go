...
func (_q *TrackQuery) QueryArtist() *ArtistQuery {
...
func (_q *TrackQuery) WithArtist(opts ...func(*ArtistQuery)) *TrackQuery {
	query := (&ArtistClient{config: _q.config}).Query()
...
func (_q *TrackQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Track, error) {
	...
	if query := _q.withArtist; query != nil {
		if err := _q.loadArtist(ctx, query, nodes, nil,
			func(n *Track, e *Artist) { n.Edges.Artist = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *TrackQuery) loadArtist(ctx context.Context, query *ArtistQuery, nodes []*Track, init func(*Track), assign func(*Track, *Artist)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Track)
	for i := range nodes {
		if nodes[i].artist_tracks == nil {
			continue
		}
		fk := *nodes[i].artist_tracks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "artist_tracks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
