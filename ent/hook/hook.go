...
func (f LastFMAuthFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.LastFMAuthMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.LastFMAuthMutation", m)
}

// The ListenFunc type is an adapter to allow the use of ordinary
// function as Listen mutator.
type ListenFunc func(context.Context, *ent.ListenMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ListenFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ListenMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f SpotifyAuthFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SpotifyAuthMutation); ok {
		return f(ctx, mv)
	}
