...
func (_q *AlbumQuery) QueryUser() *UserQuery {
...
func (_q *AlbumQuery) QueryImages() *AlbumImageQuery {
	query := (&AlbumImageClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *AlbumQuery) QueryListens() *ListenQuery {
...
func (_q *AlbumQuery) WithUser(opts ...func(*UserQuery)) *AlbumQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *AlbumQuery) WithImages(opts ...func(*AlbumImageQuery)) *AlbumQuery {
	query := (&AlbumImageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AlbumQuery) WithListens(opts ...func(*ListenQuery)) *AlbumQuery {
	query := (&ListenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AlbumQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Album, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Album, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withImages; query != nil {
		if err := _q.loadImages(ctx, query, nodes,
			func(n *Album) { n.Edges.Images = []*AlbumImage{} },
			func(n *Album, e *AlbumImage) { n.Edges.Images = append(n.Edges.Images, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withListens; query != nil {
		if err := _q.loadListens(ctx, query, nodes,
			func(n *Album) { n.Edges.Listens = []*Listen{} },
			func(n *Album, e *Listen) { n.Edges.Listens = append(n.Edges.Listens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AlbumQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Album, init func(*Album), assign func(*Album, *User)) error {
...
func (_q *AlbumQuery) loadImages(ctx context.Context, query *AlbumImageQuery, nodes []*Album, init func(*Album), assign func(*Album, *AlbumImage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Album)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.AlbumImage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(album.ImagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.album_images
		if fk == nil {
			return fmt.Errorf(`foreign-key "album_images" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "album_images" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *AlbumQuery) loadListens(ctx context.Context, query *ListenQuery, nodes []*Album, init func(*Album), assign func(*Album, *Listen)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Album)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Listen(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(album.ListensColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.album_listens
		if fk == nil {
			return fmt.Errorf(`foreign-key "album_listens" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "album_listens" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
