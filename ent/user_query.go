...
// QuerySpotifyAuth chains the current query on the "spotify_auth" edge.
func (_q *UserQuery) QuerySpotifyAuth() *SpotifyAuthQuery {
	query := (&SpotifyAuthClient{config: _q.config}).Query()
...
// QueryLastfmAuth chains the current query on the "lastfm_auth" edge.
func (_q *UserQuery) QueryLastfmAuth() *LastFMAuthQuery {
	query := (&LastFMAuthClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithSpotifyAuth tells the query-builder to eager-load the nodes that are connected to
// the "spotify_auth" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSpotifyAuth(opts ...func(*SpotifyAuthQuery)) *UserQuery {
	query := (&SpotifyAuthClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSpotifyAuth = query
	return _q
}

// WithLastfmAuth tells the query-builder to eager-load the nodes that are connected to
// the "lastfm_auth" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithLastfmAuth(opts ...func(*LastFMAuthQuery)) *UserQuery {
	query := (&LastFMAuthClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withSpotifyAuth; query != nil {
		if err := _q.loadSpotifyAuth(ctx, query, nodes, nil,
			func(n *User, e *SpotifyAuth) { n.Edges.SpotifyAuth = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadSpotifyAuth(ctx context.Context, query *SpotifyAuthQuery, nodes []*User, init func(*User), assign func(*User, *SpotifyAuth)) error {
	...
	query.withFKs = true
	query.Where(predicate.SpotifyAuth(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SpotifyAuthColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_spotify_auth
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_spotify_auth" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_spotify_auth" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadLastfmAuth(ctx context.Context, query *LastFMAuthQuery, nodes []*User, init func(*User), assign func(*User, *LastFMAuth)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.LastFMAuth(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.LastfmAuthColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_lastfm_auth
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_lastfm_auth" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *UserQuery) loadListens(ctx context.Context, query *ListenQuery, nodes []*User, init func(*User), assign func(*User, *Listen)) error {
	...
	query.Where(predicate.Listen(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ListensColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_listens
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_listens" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
